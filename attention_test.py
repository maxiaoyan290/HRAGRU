...
device = 'cuda'
mymodel = attention_model.AttentionGraph().to(device)
mymodel.load_state_dict(torch.load('model_attention.pth'))
...
def get_Data(corporate_name):
    ...
    for i in range(0, len(data_df)):
        ...
        policy_time = pd.to_datetime(target_time).to_period('M')

        selected_policy = policy_df[policy_df['时间'] == policy_time]
        # 检查是否找到了数据
        ...
        stock_df = stock_df.unsqueeze_(0)
        financial_df = financial_df.unsqueeze_(0)
        macro_df = macro_df.unsqueeze_(0)
        selected_df_p = selected_df_p.unsqueeze_(0)
        selected_df = selected_df.unsqueeze_(0)
        stock_df = stock_df.to(device)
        financial_df = financial_df.to(device)
        macro_df = macro_df.to(device)
        selected_df = selected_df.to(device)
        selected_df_p = selected_df_p.to(device)

        output, logits = mymodel(stock_df, financial_df, macro_df, selected_df, selected_df_p)
    ...
    df.to_excel('graph_daily/' + corporate_name + '.xlsx', index=False)
    return
