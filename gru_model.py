...
class DeepDifferentGru(nn.Module):
    def __init__(self, input_size):
        ...
        self.gru_1 = GRUDifferent(input_size, 32, 1)
        self.gru_2 = GRUDifferent(32, 8, 1)
        self.gru_3 = GRUDifferent(64, 32, 1)
    ...
    def forward(self, x):
        ...
        h1 = torch.zeros(1, x.size(0), 512).to(device)
        out_2 = self.gru_2(out_1)
        out_2 = self.dropout(out_2).to(device)
        h2 = torch.zeros(1, x.size(0), 256).to(device)
        out = self.linear_3(out_2[:, -1, :])
        # out_3 = self.gru_3(out_2)
        # out_3 = self.dropout(out_3).to(device)
        # out_4 = self.linear_1(out_3[:, -1, :])
        # out_4 = torch.tanh_(out_4)
        # out_5 = self.linear_2(out_4)
        # out_5 = torch.tanh_(out_5)
        # out = self.linear_3(out_5)
        return out
...
class trainmodel(nn.Module):
    def __init__(self, input_size):
        ...
        self.gru = xLSTM(input_size=input_size, layers=['s', 'm', 's'])
