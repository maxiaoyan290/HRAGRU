...
mymodelnew = gru_model.DeepDifferentGru(input_size=87).to(device)
name = 'cnn'
...
for i in range(epoch):
    ...
    for data, label, news, policy, wobert, wognn in dataloader:
        ...
        output = mymodelnew(data)
        # output = mymodelnew(data)[0]
        # print(output.shape)
            # print(output.shape)
            # print(output.shape)
...
# torch.save(mymodel.state_dict(), 'model_state_dict_' + 'gru' + '.pth')
    if i == 100:
        torch.save(mymodelnew.state_dict(), 'model_state_dict_' + name + '_100' + '.pth')
    if i == 200:
        torch.save(mymodelnew.state_dict(), 'model_state_dict_' + name + '_200' + '.pth')
    if i == 300:
        torch.save(mymodelnew.state_dict(), 'model_state_dict_' + name + '_300' + '.pth')
    if i == 400:
        torch.save(mymodelnew.state_dict(), 'model_state_dict_' + name + '_400' + '.pth')
    if i == 500:
        torch.save(mymodelnew.state_dict(), 'model_state_dict_' + name + '_500' + '.pth')