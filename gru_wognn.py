...
mymodelgnn = gru_model.testmodelwithoutgnn(97).to(device)
...
optimizer = torch.optim.Adam(mymodelgnn.parameters(), lr=0.000164)
...
for i in range(epoch):
    ...
    for data, label, news, policy, wobert, wognn in dataloader:
        ...
        output = mymodelgnn(wognn, news, policy)
            #print(output.shape)
            # print(output.shape)
            # print(output.shape)
...
# torch.save(mymodel.state_dict(), 'model_state_dict_' + 'gru' + '.pth')
    if i == 50:
        torch.save(mymodelgnn.state_dict(), 'model_state_dict_' + 'wognn_gru_50' + '.pth')
    if i == 75:
        torch.save(mymodelgnn.state_dict(), 'model_state_dict_' + 'wognn_gru_75' + '.pth')
    if i == 100:
        torch.save(mymodelgnn.state_dict(), 'model_state_dict_' + 'wognn_gru_100' + '.pth')
    if i == 200:
        torch.save(mymodelgnn.state_dict(), 'model_state_dict_' + 'wognn_gru_200' + '.pth')
    if i == 300:
        torch.save(mymodelgnn.state_dict(), 'model_state_dict_' + 'wognn_gru_300' + '.pth')